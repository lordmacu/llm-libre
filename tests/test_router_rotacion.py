...
from llm_libre.router import ordenar, BANDA_EMPATE
...
def test_sin_aleatorio_el_orden_sigue_siendo_determinista():
    """Los 447 tests que ya existian llaman ordenar() con 4 argumentos."""
    rutas = [r("a"), r("b"), r("c")]
    met = {"kilo/a": m(), "kilo/b": m(), "kilo/c": m()}
    primeras = {tuple(x.clave for x in ordenar(rutas, met, Pedido(), 0.0)) for _ in range(30)}
...
def test_con_aleatorio_las_empatadas_rotan():
    ...
    vistas = {ordenar(rutas, met, Pedido(), 0.0, random.Random(s))[0].clave for s in range(40)}
...
def test_una_ruta_fuera_de_la_banda_nunca_gana():
    ...
    for s in range(60):
        salida = ordenar(rutas, met, Pedido(), 0.0, random.Random(s))
...
def test_puntajes_en_cero_no_rompen_la_banda():
    ...
    salida = ordenar(rutas, met, Pedido(), 0.0, random.Random(1))
    assert len(salida) == 2
...
def test_el_reparto_es_razonablemente_parejo():
    ...
    cuenta = {}
    for s in range(4000):
        g = ordenar(rutas, met, Pedido(), 0.0, random.Random(s))[0].clave
