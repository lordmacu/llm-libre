...
def r(modelo, provider="kilo", tier="gratis", tools=True, vision=False, context=100000):
    return Route(provider, modelo, tier,
                Capabilities(tools=tools, vision=vision, context=context, max_output=4096))


def m(quality=0.8, reliability=0.9, ttft=500, cooldown=0.0, medida_en=1000.0):
    return Metrics(quality, reliability, ttft, cooldown, medida_en)


def test_descarta_las_rutas_que_no_soportan_tools_cuando_se_piden():
    rutas = [r("con:free", tools=True), r("sin:free", tools=False)]
    salida = order_routes(rutas, {}, RouteRequest(needs_tools=True), now=0.0)
    assert [x.model_id for x in salida] == ["con:free"]


def test_descarta_las_rutas_sin_vision_cuando_se_pide():
    rutas = [r("ve:free", vision=True), r("ciego:free", vision=False)]
    salida = order_routes(rutas, {}, RouteRequest(needs_vision=True), now=0.0)
    assert [x.model_id for x in salida] == ["ve:free"]


def test_descarta_las_rutas_con_contexto_insuficiente():
    rutas = [r("grande:free", context=200000), r("chico:free", context=8000)]
    salida = order_routes(rutas, {}, RouteRequest(min_context=100000), now=0.0)
    assert [x.model_id for x in salida] == ["grande:free"]


def test_ordena_por_puntaje_descendente():
    rutas = [r("malo:free"), r("bueno:free")]
    metricas = {"kilo/malo:free": m(quality=0.3), "kilo/bueno:free": m(quality=0.95)}
    salida = order_routes(rutas, metricas, RouteRequest(), now=0.0)
    assert [x.model_id for x in salida] == ["bueno:free", "malo:free"]


def test_las_rutas_de_pago_van_siempre_al_final_aunque_puntuen_mejor():
    rutas = [r("MiniMax-M3", provider="minimax", tier="pago"), r("flojo:free")]
    metricas = {"minimax/MiniMax-M3": m(quality=1.0, reliability=1.0, ttft=100),
...
def test_permitir_pago_falso_saca_las_rutas_de_pago():
    rutas = [r("MiniMax-M3", provider="minimax", tier="pago"), r("g:free")]
    salida = order_routes(rutas, {}, RouteRequest(allow_paid=False), now=0.0)
...
def test_excluye_las_rutas_en_cooldown_pero_no_las_vencidas():
    ...
    salida = order_routes(rutas, metricas, RouteRequest(), now=100.0)
    assert [x.model_id for x in salida] == ["vencida:free"]


def test_un_modelo_explicito_filtra_pero_conserva_los_dos_proveedores():
    rutas = [r("comun:free", provider="kilo"),
             r("comun:free", provider="openrouter"),
             r("otro:free", provider="kilo")]
    salida = order_routes(rutas, {}, RouteRequest(model="comun:free"), now=0.0)
    assert len(salida) == 2
    assert {x.provider for x in salida} == {"kilo", "openrouter"}


def test_sin_candidatas_devuelve_lista_vacia():
    salida = order_routes([r("sin:free", tools=False)], {}, RouteRequest(needs_tools=True), now=0.0)
    assert salida == []
...
def test_una_ruta_sin_metricas_usa_las_neutras_y_no_ceros():
    ...
    # kilo/nueva:free (sin metricas) puntua con NEUTRAL_METRICS (0.6, 0.8, 1500):
    # 0.6*0.8*factor_latencia(1500) = 0.24, asi que le gana a la conocida y queda primera.
    # Si el fallback fuera Metrics(0,0,0,0) puntuaria 0 y quedaria de ULTIMA: el orden
    ...
    metricas = {"kilo/conocida:free": m(quality=0.3, reliability=0.5, ttft=1500,
                                        medida_en=None)}
    salida = order_routes(rutas, metricas, RouteRequest(), now=0.0)
    assert [x.model_id for x in salida] == ["nueva:free", "conocida:free"]
...
def test_una_ruta_nunca_sondeada_va_despues_de_una_con_calidad_medida():
    ...
    metricas = {
        ...
        "kilo/medida:free": m(quality=0.35, ttft=500, medida_en=1000.0),
        "kilo/nueva:free": m(quality=0.6, ttft=200, medida_en=None),
    }
    salida = order_routes(rutas, metricas, RouteRequest(), now=0.0)
    assert [x.model_id for x in salida] == ["medida:free", "nueva:free"]


def test_una_ruta_nunca_sondeada_sigue_en_la_cadena_para_poder_medirse():
    rutas = [r("nueva:free"), r("medida:free")]
    metricas = {"kilo/medida:free": m(quality=0.9, medida_en=1000.0)}
    salida = order_routes(rutas, metricas, RouteRequest(), now=0.0)
    assert "nueva:free" in [x.model_id for x in salida]
...
def test_entre_dos_nunca_sondeadas_sigue_mandando_el_puntaje():
    ...
    salida = order_routes(rutas, metricas, RouteRequest(), now=0.0)
    assert [x.model_id for x in salida] == ["rapida:free", "lenta:free"]


# --- Task 13: `prioridad`, un concepto DISTINTO de `tier` y de `perfil`. ---

def _rp(modelo, priority, provider="kilo", tier="gratis", tools=True):
    return Route(provider, modelo, tier,
...
def test_la_prioridad_ordena_dentro_del_mismo_tier_por_encima_del_puntaje():
    ...
    rutas = [_rp("chatgpt:free", 0, provider="chatgpt"), _rp("normal:free", 1)]
    metricas = {"chatgpt/chatgpt:free": m(quality=0.3, reliability=0.3, ttft=3000),
                "kilo/normal:free": m(quality=0.99, reliability=0.99, ttft=50)}
    salida = order_routes(rutas, metricas, RouteRequest(), now=0.0)
    assert [x.model_id for x in salida] == ["chatgpt:free", "normal:free"]
...
def test_la_prioridad_no_rompe_el_invariante_de_pago_al_final():
    ...
    rutas = [_rp("MiniMax-M3", 0, provider="minimax", tier="pago"),
             _rp("mediocre:free", 100)]
    metricas = {"minimax/MiniMax-M3": m(quality=1.0, reliability=1.0, ttft=50),
                "kilo/mediocre:free": m(quality=0.2, reliability=0.3, ttft=5000)}
    salida = order_routes(rutas, metricas, RouteRequest(), now=0.0)
    assert [x.tier for x in salida] == ["gratis", "pago"]
    assert [x.model_id for x in salida] == ["mediocre:free", "MiniMax-M3"]
...
def test_una_ruta_en_cooldown_se_salta_aunque_tenga_la_maxima_prioridad():
    ...
    salida = order_routes(rutas, metricas, RouteRequest(), now=100.0)
    assert [x.model_id for x in salida] == ["normal:free"]
...
def test_un_modelo_explicito_se_sirve_aunque_no_sea_el_de_mayor_prioridad():
    ...
    rutas = [_rp("prioritario:free", 0, provider="chatgpt"),
             _rp("elegido-a-mano:free", 1)]
    salida = order_routes(rutas, {}, RouteRequest(model="elegido-a-mano:free"), now=0.0)
    assert [x.model_id for x in salida] == ["elegido-a-mano:free"]
...
def test_una_peticion_con_tools_nunca_enruta_a_chatgpt():
    ...
    salida = order_routes([chatgpt, kilo], {}, RouteRequest(needs_tools=True), now=0.0)
    assert [x.model_id for x in salida] == ["con-tools:free"]
...
def test_una_peticion_sin_tools_prefiere_chatgpt_por_su_prioridad():
    ...
    salida = order_routes([chatgpt, kilo], {}, RouteRequest(), now=0.0)
    assert [x.model_id for x in salida] == ["gpt-5-3-mini", "normal:free"]


def test_un_id_de_chatgpt_elegido_a_mano_sigue_ruteando_directo():
    chatgpt = _rp("gpt-5-3-mini", 0, provider="chatgpt", tools=False)
    kilo = _rp("otro:free", 1)
    salida = order_routes([chatgpt, kilo], {}, RouteRequest(model="gpt-5-3-mini"), now=0.0)
    assert [x.model_id for x in salida] == ["gpt-5-3-mini"]
...
def test_una_ruta_de_chatgpt_descubierta_de_verdad_rutea_directo_al_pedirla():
    ...
    salida = order_routes(descubiertas, {}, RouteRequest(model="gpt-5-3-mini"), now=0.0)
    assert [x.model_id for x in salida] == ["gpt-5-3-mini"]
