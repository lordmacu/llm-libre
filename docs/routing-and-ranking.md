...
`openrouter` (`prioridad: 1`) -- but it is still `tier: gratis`, so this has
nothing to do with money, and `minimax` (`tier: pago`, `prioridad: 2`) is
still tried dead last regardless of that low-looking number.

## How the router orders candidates

`router.ordenar()` (called on every request that resolves to `auto*`; an
explicit real model id skips all of this and goes straight to that model's
routes) does, in order:
...
1. **Filter by capability.** A route is a candidate at all only if it
   ...
   OpenAPI docs at `/docs` for that last one, it is easy to miss), and a
   context window at least as large as `x_min_contexto` if set. If nothing
   in the whole catalog can ever satisfy this, the request gets `400`
...
- **`en_cooldown_hasta` is a raw Unix timestamp in seconds** (`0` = not in
  ...
  time (`date -r <value>` on macOS/Linux, or paste it into any epoch
  converter).
...
Critically, as of the current design, **client traffic alone never sets a
cooldown directly.** A run of failures against a free route only builds up
an internal *suspicion* counter; crossing a threshold makes the gateway
fire its own confirmation probe (a small, fixed payload the gateway itself
authored) in the background, and only that probe's result decides whether
the route actually gets punished. This exists specifically so that a
client cannot cool down a healthy route just by sending it requests that
happen to fail for reasons that have nothing to do with the route being
broken (a huge prompt, a moderation flag, a reasoning model that burns its
whole token budget without answering...) -- see the design spec §7 for the
long history of why this changed from "the client's own failures count
directly" to "the client can only make the gateway go check."

**Clearing a cooldown needs no operator action**: it expires on its own
once `en_cooldown_hasta` is in the past, or gets cleared early the moment
that route serves a real success or passes a fresh probe. There is
currently no manual "un-cooldown this route" switch -- if a route is stuck
punished longer than its backoff should allow, that itself is worth
investigating (see the runbook).
